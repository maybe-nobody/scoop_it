...
class ActorCritic(nn.Module):
    ...
    def act(self, obs_dict):#根据当前的观察，通过神经网络生成一个概率分布，并从中随机抽取**一个动作。
        # used specifically to collection samples during training
        # it contains exploration so needs to sample from distribution
        mu, logstd, value = self._actor_critic(obs_dict)
        sigma = torch.exp(logstd)
        distr = torch.distributions.Normal(mu, sigma)#构建一个高斯分布，均值就是mu方差就是sigma，构建的高斯分布是多维的，维数就是action的维数，维数是由mu的维数所决定的
