...
class PPO_Catch_TwoStage(object):
    ...
    def train_epoch(self):
        ...
        for mini_ep in range(0, self.mini_epochs_num):
            ...
            for i in range(len(self.storage)):
                ...
                obs_track = self.running_mean_std_track(obs[:,:-2])
                obs_hand = self.running_mean_std_hand(obs[:,-2:])
    ...
    def model_act(self, obs_dict, inference=False):
        processed_obs_track = self.running_mean_std_track(obs_dict['obs'][:, :-2])
        processed_obs_hand = self.running_mean_std_hand(obs_dict['obs'][:, -2:])
        processed_obs = torch.cat((processed_obs_track, processed_obs_hand), dim=1)
        input_dict = {
            'obs': processed_obs,
            'obs_t': processed_obs[:,:-2],
            'obs_c': processed_obs[:,3:],
        }
        if not inference:
