...
def calculate_arm_Te(pose, quate):#顺序是wxyz
    """
...
def get_total_dimension(data):
    ...
    if isinstance(data, spaces.Dict) or isinstance(data, dict):#用来判断一个对象是不是某个类，是不是spaces.Dict，是不是dict
        for value in data.values():
            total_dimension += get_total_dimension(value)#计算value的维数
    ...
    return total_dimension#计算一个嵌套的观测空间（observation space）或动作空间（action space）的总维度
